        ...
        <div className="-mx-1 mb-3 flex gap-1.5 overflow-x-auto px-1 pb-1">
          {jornada.jogos.map(j => (
            <Link key={j.id} to="/jogo/$id" params={{ id: j.id }}
              ...
              <TeamBadge code={j.home.code} flag={j.home.flag} name={j.home.name} monogram={(j.home as any).monogram} crest={(j.home as any).crest_url} size="sm" />
              <span className="text-[10px] font-bold text-white/50">v</span>
              <TeamBadge code={j.away.code} flag={j.away.flag} name={j.away.name} monogram={(j.away as any).monogram} crest={(j.away as any).crest_url} size="sm" />
