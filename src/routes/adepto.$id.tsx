...
import { ArrowLeft, Trophy, Target, Percent, Users, Shield, ChevronDown, ChevronUp } from "lucide-react";
...
function PredictionsList({ predictions }: { predictions: any[] }) {
  ...
  const visible = expanded ? predictions : predictions.slice(0, 5);
    ...
    <div className="mx-5 mt-8 md:mx-8">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded-full bg-secondary px-2 py-0.5 text-[11px] text-muted-foreground">{predictions.length}</span>
        </div>
      </div>
      ...
      {predictions.length > 5 && (
        <button
          ...
          className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl border border-border py-2.5 text-sm text-muted-foreground hover:bg-accent/50 transition-smooth"
        >
          {expanded ? <><ChevronUp className="h-4 w-4" /> Ver menos</> : <><ChevronDown className="h-4 w-4" /> Ver todas ({predictions.length - 5} mais)</>}
        </button>
      )}
    </div>
  );
