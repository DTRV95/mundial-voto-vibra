  ...
  const { data: prognosticos = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase as any)
        .from("prognosticos")
        .select("id,suggestion,summary,created_at,match:match_id(id,kickoff_at,home:home_team_id(name,flag,code,monogram,crest_url),away:away_team_id(name,flag,code,monogram,crest_url),round:round_id(label))")
        .eq("published", true)
